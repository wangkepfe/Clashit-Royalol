// smart v11 — REJECTED branch, kept for comparison. v10 + enemy-elixir-timed
// commitment (HANDOFF §7.2 "exploit a starved opponent"): when v.enemy.elixir
// is low and I have an elixir edge, commit support to the lead immediately
// instead of waiting for it to advance. Measured 50% vs the v10 mirror with
// LOWER tower damage — a single tactical lever does not crack the coinflip
// mirror. Selectable so the (non-)effect is visible/reproducible.
import { myUnits, enemyUnits, myTowers, enemyTowers } from './lib.js';

export default function smartV11(v) {
  const me = v.self;
  const E = me.elixir;
  ...
  // v11 lever: opponent elixir-starved and I have an edge → commit support now.
  const enemyBroke = v.enemy.elixir <= 3.5 && E >= v.enemy.elixir + 3;

  if (!myGiant && has('Giant') && E >= 7) {
    return troop('Giant', pushX, targetSide, backY);
  }
  if (lead && (leadAdvanced || enemyBroke) && E >= 4) {
